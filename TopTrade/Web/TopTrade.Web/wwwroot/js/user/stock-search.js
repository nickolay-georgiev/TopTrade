function init() {
    ...
    var domElements = {
        ...
        searchInputValue: () => domElements.searchMenu().querySelector('input').value.trim(),
    ...
    async function stockSearch() {
        const inputValue = domElements.searchInputValue();
    ...
    async function addStockToWatchList(stock) {
        ...
        var htmlElement = [...document.querySelectorAll('.table-div tbody tr')]
            .find(x => x.querySelector('.stock-ticker').textContent == stock.ticker);
    ...
    function configureOrder(event) {
        ...
        const companyLogo = `//logo.clearbit.com/${stockName}.com`;
        ...
        stockQuantityElement.addEventListener('input', (e) => {
            ...
            executeOrderButton.addEventListener('click', async (event) => {
                ...
                const response = await makeAjax(endpoint, tradeDetails);
                let result = await response.json();

                if (response.ok) {
                    document.querySelector('.available-funds > span').textContent = result.available.toFixed(2);
                    document.querySelector('.allocated-funds > span').textContent = result.totalAllocated.toFixed(2);
                    document.querySelector('.profit-funds span').textContent = `$${result.profit.toFixed(2)}`;
                    document.querySelector('.equity-funds > span').textContent = result.equity.toFixed(2);
                    domElements.configOrderQuantity().value = '';
                    error.hidden = true;
                    document.querySelector('.close').click();


                    //var htmlElement = [...document.querySelectorAll('.table-div tbody tr')].reduce((acc, value) => {
